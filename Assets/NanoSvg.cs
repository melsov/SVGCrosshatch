...
namespace NanoSvg
{
    ...
    {   
        public class SvgPath
        {
            ...
            public void updateWith(CSSClass css) {
                if(css.hasFill) {
                    fillColor = ParseColor(css.fill);
                    hasFill = true;
                }
                if(css.hasStroke) {
                    strokeColor = ParseColor(css.stroke);
                    hasStroke = true;
            ...
            public override string ToString() {
                ...
                foreach(float p in pts) {
                    pointsStr = string.Format("{0} {1}", pointsStr, p);
                }
                return string.Format("SvgPath: {0} numPoints: {1} | {2} ", _class, pts.Length, pointsStr);
            }

            public SvgPath Clone() {
                return CloneWithPoints(pts);
            }

            public SvgPath CloneWithPoints(float[] points) {
                SvgPath s = new SvgPath();
                s.id = id;
                s._class = _class;
                s.pts = new float[points.Length];
                for(int i=0; i<points.Length; ++i) {
                    s.pts[i] = points[i];
                }
                s.npts = s.pts.Length / 2;
            ...
            public Vector2f this[int i] {
                get {
                    return new Vector2f(pts[i * 2], pts[i * 2 + 1]);
                }
                set {
                    pts[i * 2] = value.x;
                    pts[i * 2 + 1] = value.y;
            ...
            public SvgPath ReversedPointsClone() {
                return CloneWithPoints(ReversedPoints());
            }
            ...
            private float[] ReversedPoints() {
                float[] rev = new float[pts.Length];
                Vector2f v;
            ...
            public SvgPath ShiftedPointsClone(Vector2f shift) {
                return CloneWithPoints(ShiftedPoints(shift));
            }

            public void ShiftPoints(Vector2f shift) {
                pts = ShiftedPoints(shift);
            }

            private float[] ShiftedPoints(Vector2f shift) {
                float[] shifted = new float[pts.Length];
                Vector2f v;
                for(int i=0; i<npts; ++i) {
                    v = this[i] + shift;
                    shifted[i * 2] = v.x;
            ...
            public static SvgPath RotatedClone(SvgPath path, Matrix2f mat) {
                SvgPath clone = path.Clone();
                clone.Rotate(mat);
                return clone;
            }

            public void Rotate(Matrix2f mat) {
                for(int i=0; i < npts; ++i) {
                    this[i] = mat * this[i];
                }
            }
