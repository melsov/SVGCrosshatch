...
namespace g3
{
	public struct Segment2d // : IParametricCurve2d
    {
        ...
        public bool Intersects(ref Segment2d seg2, double dotThresh = double.Epsilon, double intervalThresh = 0)
        {
            ...
            if (System.Math.Abs(D0DotPerpD1) > dotThresh) {   // Lines intersect in a single point.
                ...
                return System.Math.Abs(s) <= (Extent + intervalThresh) 
                        && System.Math.Abs(s2) <= (seg2.Extent + intervalThresh);
            }

            // Lines are parallel.
            diff.Normalize();
            double diffNDotPerpD1 = diff.DotPerp(seg2.Direction);
            if (System.Math.Abs(diffNDotPerpD1) <= dotThresh) {
